...
pacman::p_load("tidyverse","flextable","lubridate","leaflet","DT",'widgetframe')
...
```{r update,echo=FALSE,out.width = "100%"}
d_1=dt_final%>% filter(dt$ultima_atualizacao==dt$data_cadastro) %>% 
  ...
  arrange(data_cadastro) %>% mutate(data_cadastro=format(data_cadastro,"%d/%m/%Y"),
                                          ultima_atualizacao=format(ultima_atualizacao,"%d/%m/%Y")) %>% datatable(colnames = c(" Domínio","UF","Data de Cadastro", "Última Atualização"), rownames=FALSE,                                         options = list(
...
```{r orgaos,echo=FALSE, message=FALSE,warning=FALSE,out.width = "100%"}
d_2=dt_final%>% mutate(nome=toupper(rm_accent(nome))) %>% count(nome, sort = T) %>% datatable(colnames = c(" Órgão","Quantidade"),rownames=FALSE,options = list(
  language = list(url = '//cdn.datatables.net/plug-ins/1.10.19/i18n/Portuguese-Brasil.json'),
  pageLength = 5
))
...
```{r top10,echo=FALSE, message=FALSE,warning=FALSE,out.width = "100%"}
  ...
  d_3=dt_final %>% count(cidade,uf, sort=T) %>% datatable(colnames = c(" Cidade","UF","Quantidade"),rownames=FALSE,options = list(
  language = list(url = '//cdn.datatables.net/plug-ins/1.10.19/i18n/Portuguese-Brasil.json'),
  pageLength = 5))
