...
d_1=dt_final%>% filter(dt$ultima_atualizacao==dt$data_cadastro) %>% 
  ...
  arrange(data_cadastro) %>% mutate(data_cadastro=format(data_cadastro,"%d/%m/%Y"),
                                          ultima_atualizacao=format(ultima_atualizacao,"%d/%m/%Y")) %>% datatable(colnames = c(" Domínio","UF","Data de Cadastro", "Última Atualização"),                                         options = list(
...
d_2=dt_final%>% count(nome, sort = T) %>% datatable(colnames = c(" Órgão","Quantidade"),options = list(
  language = list(url = '//cdn.datatables.net/plug-ins/1.10.19/i18n/Portuguese-Brasil.json'),
  pageLength = 5
))
